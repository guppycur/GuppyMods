...
/// <summary>
/// Class for block entity wave spawner and buff and damage effects
/// by Mortelentus
/// Edited by StompyNZ
/// </summary>
public class BlockMortSpawner : BlockLoot
{
  ...
  private static bool IsLooted(byte _metadata)
  {
  ...
  public override int OnBlockDamaged(WorldBase _world, int _clrIdx, Vector3i _blockPos, BlockValue _blockValue, int _damagePoints, int _entityIdThatDamaged, bool _bUseHarvestTool, int _recDepth)
  {
    ...
    if (isMultiBlock && _blockValue.ischild)
    {
      ...
      return block.ischild ? 0 : list[block.type].OnBlockDamaged(_world, _clrIdx, parentPos, block, _damagePoints, _entityIdThatDamaged, false, 0);
    }

    var d = _blockValue.damage;
    var max = list[_blockValue.type].MaxDamage;

    if (d >= max || d + _damagePoints < max) return base.OnBlockDamaged(_world, _clrIdx, _blockPos, _blockValue, _damagePoints, _entityIdThatDamaged, _bUseHarvestTool, _recDepth);
  ...
  {
    if (!Stopped(_blockValue.meta2) || IsLooted(_blockValue.meta2))
    {
      return false;
    }
...
/*     RadiationDamage = 0;
    if (props.ContainsKey(PropRadiationDamage) && !uint.TryParse(props[PropRadiationDamage], out RadiationDamage)) Debug.Log("Unable to parse " + PropRadiationDamage + " as an int in " + blockName);
 */
/*     //buffs
  ...
  private void Update()
  {
    ...
    if (CheckRadius > 0 && (NumberToSpawn > 0) && MaxSpawn > 0 && CheckArea > 0)
...
/*         if (RadiationArea > 0 && RadiationArea >= distance) DoBuffsAndRadiation(player); */
    ...
    if (spawnNeeded && numberSpawned < MaxSpawn)
    {
      ...
      for (var i = 0; i < NumberToSpawn; ++i)
      {
        ...
        int y;
        int z;
        if (!world.FindRandomSpawnPointNearPosition(Position.ToVector3(), 15, out x, out y, out z, areaSize, true, false)) continue;

        var entityId = EntityGroups.GetRandomFromGroup(EntityGroup);
    ...
    if (PauseTimeMinutes < 999) return;

    BlockValue.meta2 = (byte)(BlockValue.meta2 | 2);
    GameManager.Instance.World.SetBlockRPC(Position, BlockValue);
  }

/*   private void DoBuffsAndRadiation(EntityAlive player)
  {
    if (RadiationDamage > 0) player.DamageEntity(DamageSource.radiation, (int)RadiationDamage, false, 1f);
